<?php
	...
	if(isset($_GET['edit']) && isset($_GET['id'])){
		if($_GET['edit']=="news" && $_GET['id']!=''){
			$qry = "SELECT `id`,`title`,`content`,`dateadded`,`imagepath`,`isfeatured` FROM tblnews WHERE `id`=".$_GET['id'];
			...
			$isfeat = $row['isfeatured']=="1"?"checked":"";
			$read = $row['isfeatured']=="1"?"disabled":"";
...
<div class="admin-news">
    ...
    <form class="news-form" onsubmit="return false;">
		...
		<input name = "markasfeautured" type="checkbox" id="markasfeautured" <?php echo $isfeat." ".$read; ?> />
		<label for="markasfeautured">Mark as Feautured</label>
        <textarea id="news-description" name="description"  ><?php echo $content; ?></textarea>
        <?php echo $btn; ?>
    </form>
    <?php
    $display = (isset($_GET['edit']) && isset($_GET['id']) && $_GET['edit']=="news" && $_GET['id']!='')?$_GET['id']:'';
    ...
    while($row = mysqli_fetch_assoc($result)) {
		if($display!=$row["id"]){ ?>
		    <div class="article_wrapper">
				...
				<button type="button" data-id="<?php echo $row["id"]; ?>" class="btn-remove-news">DELETE</button>
		        <hr>
		    </div><?php
		}
    }?>
